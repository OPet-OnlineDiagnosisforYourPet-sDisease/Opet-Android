...
fun uriToFile(selectedImg: Uri, context: Context) : File {
    ...
    //utk menampung hasil file intent sementara
    val myFile = createCustomTempFile(context)

    //menuliskan Uri ke File
...
fun String.withDateFormat(): String {
    val format = SimpleDateFormat("MMM d, yyyy, h:mm:ss a", Locale.US)
    val date = format.parse(this) as Date

    return DateFormat.getDateInstance(DateFormat.FULL).format(date)
}

fun String.getRelativeTime(context: Context): String {
    val inputFormat = SimpleDateFormat("MMM d, yyyy, h:mm:ss a", Locale.getDefault())
    val date = inputFormat.parse(this)

    val now = Calendar.getInstance().time
    val diff = now.time - date.time

    val seconds = Math.abs(diff / 1000).toInt()
